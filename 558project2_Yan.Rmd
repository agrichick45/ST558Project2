...
=======
>>>>>>> 94eb2a4de1413d5e2252bd568afd701b8ab4ba77
...
# Data plots  
scatter plots between every two variables show the possible relationship and correlation among the variables in the training data set.
```{r,fig1, fig.height = 20, fig.width = 20}
train_1<-train%>%mutate(shares_log=log(shares))
train_2<-train_1[,-c(25:32,54:55)]
library(PerformanceAnalytics)
names(train_2)
# scatter plots between variables
chart.Correlation(train_2[,c(1:9,46)], histogram = TRUE, method = "pearson")
chart.Correlation(train_2[,c(10:19,46)], histogram = TRUE, method = "pearson")
chart.Correlation(train_2[,c(20:29,46)], histogram = TRUE, method = "pearson")
chart.Correlation(train_2[,c(30:39,46)], histogram = TRUE, method = "pearson")
chart.Correlation(train_2[,c(40:45,46)], histogram = TRUE, method = "pearson")
```
...
We can inspect the trend of shares as a function of num_images. If the points show an upward trend, then articles with more images tend to be shared more often. If we see a negative trend then articles with more images tend to be shared less often.
```{r}
...
train_back <- train(log(shares) ~ ., 
                   ...
                   tuneGrid = expand.grid(nvmax=1:53),
...
Tune parameters must be chosen ??, B and d in the boosting tree model.
```{r}
...
```{r}
comparison<-data.frame(RMSE_lm, RMSE_back, RMSE_boost)
